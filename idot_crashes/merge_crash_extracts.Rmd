...
# Prep crash data by wards, community areas
  ...
  #commarea_crashes <- crashes_geo %>%
  #  count(commarea,most_severe_injury)
  
 # ca_summary = crashes_geo %>%
 #   st_drop_geometry() %>%
 #   mutate(crash_date = as.POSIXct(crash_date), year = format(crash_date,"%Y")) %>%
 #   group_by(commarea, year, most_severe_injury) %>%
 #   summarize(total = n()) %>%
 #   ungroup() %>%
 #   pivot_wider(names_from = most_severe_injury, values_from = total)
 #
 #   # All Crash Types
 #   write.csv(ca_summary, "IDOT_2009_2017_Summary_Community_Areas.csv")

    # All Crashes - segment Ped/Bike crashes
    ca_summary_ped_cyc = crashes_geo %>%
      ...
      #ungroup() %>%
      #pivot_wider(names_from = most_severe_injury, values_from = crashes)
